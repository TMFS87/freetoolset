...
const I18N = path.join(__dirname, 'js', 'i18n.js');
let js = fs.readFileSync(I18N, 'utf8');

// 1. 扩展 applyLang 支持 placeholder
const oldApply = `function applyLang(lang) {
    ...
    for (var i = 0; i < els.length; i++) {
      ...
      if (lang === 'zh' && Object.prototype.hasOwnProperty.call(DICT, key)) {
...
const newApply = `function applyLang(lang) {
    var els = document.querySelectorAll('[data-i18n]');
    for (var i = 0; i < els.length; i++) {
      var el = els[i];
      var key = el.getAttribute('data-i18n');
...
if (!js.includes('data-i18n-placeholder')) {
  js = js.replace(oldApply, newApply);
  console.log('Added placeholder support.');
} else {
  console.log('Placeholder support already exists.');
}
...
// 找到 DICT 的结束位置（在 footerDesc 之后或任意已知 key 后）
// 我们简单插入到 DICT 对象开头之后
const dictStart = js.indexOf('var DICT = {');
const firstEntryEnd = js.indexOf("'Home': '首页',", dictStart);
if (firstEntryEnd === -1) {
  console.error('Could not find DICT insert point');
  process.exit(1);
}

let insertPos = firstEntryEnd + "'Home': '首页',".length;

// 过滤已存在的 key
const existingKeys = new Set((js.match(/'([^']+)':/g) || []).map(s => s.slice(1, -2)));
const toAdd = [];
for (const [k, v] of Object.entries(newEntries)) {
  if (!existingKeys.has(k)) {
    toAdd.push(`    '${k.replace(/'/g, "\\'")}': '${v.replace(/'/g, "\\'")}',`);
  }
}

if (toAdd.length) {
  js = js.slice(0, insertPos) + '\n' + toAdd.join('\n') + js.slice(insertPos);
  console.log(`Added ${toAdd.length} new translations.`);
} else {
  console.log('No new translations to add.');
}

fs.writeFileSync(I18N, js);
